...
(function ($sc) {
    $sc.widget("sc.listview", {
        options: {
            page: 0,
            template : "{{each data}} <li class='group'><span class='ui-icon ui-icon-carat-1-s'></span>${n}</li>{{each s}}<li class='state' id='${id}'>${n}</li>{{/each}}{{/each}}"
        },
    ...
    _create: function () {
        ...
        this.element.delegate('li.state', 'mouseenter', function () { $sc(this).addClass("ui-state-focus"); } )
                    ...
                    .delegate('li.state', 'click', function(){  
                                ...
                                self.element.trigger('listview:change', [this.id]);
        ...
        this.element.delegate('li.group', 'click', function() {
            var header = $sc(this),
                display = '';
    ...
    handleScroll : function(){
        ...
        if (element.prop('scrollHeight') <= this.element.height()) {
            this.load.hide();
        }else{
            this.load.show();
        }
    ...
    buildLoad : function(){
        this.load = $sc("<div class='ui-list-load'><img src='/sitecore/shell/Themes/Standard/Default/WFM/ComponentArt/images/spinner.gif' width='16' height='16' border='0' /><span>" + this.options.loading + "</span></div>")
    ...
    addSearch : function(){
        ...
        this.search.width(this.width).css( { 'paddingLeft' : '10px', 'paddingRight' :'10px'})
        ...
        clear.css('left', this.search.width() - this.element.parent().width()  + 22 + "px");
