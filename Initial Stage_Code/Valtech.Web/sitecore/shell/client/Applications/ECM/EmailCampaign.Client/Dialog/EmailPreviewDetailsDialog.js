define(["sitecore", "/-/speak/v1/ecm/ServerRequest.js"], function (_sc) {
  return _sc.Definitions.App.extend({
    ...
    showDialog: function (context) {
      ...
      this.TestIdValue.set("text", root.data("sc-test-id"));

      this.setValue(this.PreviewDetails.url, this.PreviewDetails.name);
    ...
    next: function () {
      ...
      this.setValue(this.PreviewDetails.list[currentIndex].url, this.PreviewDetails.list[currentIndex].name);
    },
    previous: function () {
      var currentUrl = this.EmailPreviewImage.get("imageUrl");
      var currentIndex;
