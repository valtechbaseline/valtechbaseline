define([
  ...
  function (sitecore, MessageHelper, ServerRequest, Validation) {
    return {
      initLanguage: function (contextApp) {
        ...
        sitecore.on("change:messageContext", _.debounce(function () {
          languageSwitcher.viewModel.setSelectedLanguages(messageContext.get("languages"));

          if (
            contextApp.MessageContext.get("messageState") != MessageHelper.MessageStates.DRAFT &&
            contextApp.MessageContext.get("messageState") != MessageHelper.MessageStates.INACTIVE) {
        ...
        languageSwitcher.on("change:selectedLanguage", function () {
          ...
          if (messageContext.get("isModified") && !MessageHelper.saveMessage(messageContext, messageBar, currentLanguage, contextApp, sitecore)) {
            languageSwitcher.viewModel.setSelectedLanguage(currentLanguage);
            return;
          }

          if (!Validation.subjectIsEmpty(contextApp.MessageContext.get("variants"), contextApp.MessageBar, sitecore)) {
            languageSwitcher.viewModel.setSelectedLanguage(currentLanguage);
            return;
          }
        ...
        function switchLanguage(messageId, language) {
          ...
          ServerRequest("EXM/SwitchLanguage", {
            data: context,
