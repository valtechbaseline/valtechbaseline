define([
  ...
  /**
   * There is no explicit use of showMessagePerformanceDialog,
   *  but in case we have a referense to this dialog in items or in code generated from backend,
   *  it's better to follow backward compatibility and assign this function to a global scope
   */
  window.showMessagePerformanceDialog = showMessagePerformanceDialog;

  return {
    initReportTab: function (sitecore, contextApp) {
      ...
      contextApp.UnsubscribersListControl.on("change:items", function (e) {
        ...
        if (action) {
          if (contextApp.MessageContext.get("messageState") == MessageHelper.MessageStates.DRAFT) {
            action.disable();
            return;
          }
