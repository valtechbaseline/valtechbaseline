define(["sitecore", "/-/speak/v1/ecm/Validation.js"], function (sitecore, Validation) {
  function initGeneralTab(sitecore, contextApp) {
    ...
    contextApp.NameTextBox.on("change:text", function () {
      ...
      this.viewModel.$el.parent().removeClass("has-error");
      if (!Validation.nameIsValid(this.get("text"), messageBar, sitecore, messageContext.get("itemNameValidation"))) {
        this.viewModel.$el.parent().addClass("has-error");
      }
      Validation.isModified(messageContext, messageBar);
      sitecore.trigger("change:messageContext", this);
    });
    ...
    contextApp.FromNameTextBox.on("change:text", function () {
      ...
      this.viewModel.$el.parent().removeClass("has-error");
      if (!Validation.fromNameIsValid(this.get("text"), contextApp.FromEmailTextBox.get("text"), messageBar, sitecore)) {
        this.viewModel.$el.parent().addClass("has-error");
      }
      Validation.isModified(messageContext, messageBar);
      sitecore.trigger("change:messageContext", this);
    ...
    contextApp.FromEmailTextBox.on("change:text", function () {
      ...
      this.viewModel.$el.parent().removeClass("has-error");
      if (!Validation.fromEmailIsValid(this.get("text"), contextApp.FromNameTextBox.get("text"), messageBar, sitecore)) {
        this.viewModel.$el.parent().addClass("has-error");
      }
      Validation.isModified(messageContext, messageBar);
      sitecore.trigger("change:messageContext", this);
    ...
    contextApp.ReplyToTextBox.on("change:text", function () {
      ...
      this.viewModel.$el.parent().removeClass("has-error");
      if (!Validation.replyToTextBoxIsValid(this.get("text"), messageBar, sitecore)) {
        this.viewModel.$el.parent().addClass("has-error");
      }
      Validation.isModified(messageContext, messageBar);
      sitecore.trigger("change:messageContext", this);
  ...
  function listenToTargetDeviceComboBox(control, messageContext, messageBar, propertyName) {
    ...
    control.on("change:selectedItem", function () {
      var selectedItem = this.get("selectedItem");
      if (selectedItem != undefined && selectedItem.id != undefined && !messageContext.get("isReadonly")) {
        validateDevice(selectedItem.id, messageContext.get("messageId"), messageBar, messageContext, propertyName, control);
      }
    });
  }

  function validateDevice(deviceId, messageId, messageBar, messageContext, propertyName, control) {
    ...
    postServerRequest("EXM/ValidateDevice", { deviceId: deviceId, messageId: messageId }, function (response) {
      if (response.error) {
        var messagetoAdd = Validation.createErrorMessage(errMessageId, response.errorMessage);
        messageBar.addMessage("error", messagetoAdd);
        control.viewModel.$el.parent().addClass("has-error");
      } else {
        control.viewModel.$el.parent().removeClass("has-error");
      }
